...
B 负责 EMS/operator 侧的调度决策、本地 `ems.db` 数据层和 PyQt6 操作界面；B 是 A 的 TCP 客户端，不充当 TCP 中心。正式运行拆成三个独立进程：`B_IO` 独占 TCP，`B_COMPUTE` 只读写数据库并计算目标，GUI 只读写数据库并展示/配置。

## 当前 GUI 功能

`B_dispatch/gui_b.py` 已扩展为 B 的 SCADA/操作工作台，并保持 C 主机的浅蓝灰卡片式视觉风格。

- **运行监控**：在数据库中配置 A IP/TCP 端口并启停 `B_IO`；显示负荷、风电 available/operating limit/target/actual、柴油 target/actual、功率不平衡、风速、柴油能力余量、动态状态年龄、桨距 actual、session/step、`sampled_at_utc`。GUI 不直接打开套接字。
...
- **EMS 调度**：`B_COMPUTE` 按周期生成决策；闭环写入可执行 outbox，开环只记录/展示而绝不入发送队列。手动按钮同样只入库，由 `B_IO` 核对 session/step/状态年龄后发送并记录 ACK；delivery-unknown 禁止盲目重发。
- **历史数据**：读取 `ems.db/state_history`，按 session 过滤并支持 CSV 导出。
- **通信诊断**：显示 `B_IO/B_COMPUTE` 心跳、outbox pending/sending 数量、最近协议 seq、ACK 和事件日志。
...
5. `dispatch_outbox / process_status`：计算与通信进程的可靠交接队列，以及三个进程的状态心跳。
...
- `communication_service.py` 是唯一允许创建 `EMSTcpClient` 的正式运行进程；接收的完整 state 先入库，待发送决策从 outbox 原子领取。
- `compute_service.py` 不导入 TCP 模块，只从 `ems.db` 读取最新状态与参数，并将决策写回 outbox。
- `gui_b.py` 不直接请求 state、不发送 dispatch；连接按钮只更新 `communication_config`。
...
前两条命令会启动完整的三个独立进程；`gui`/`gui_b` 仅用于单独调试界面。Windows 也可运行 `scripts/start_b.ps1`。
...
- 默认闭环配置由 `ems_runtime_config` 持久化；开环模式只生成 `open_loop` 记录，任何情况下都不会由 `B_IO` 领取或发送。
