...
class EMSComputeService:
    ...
    def initialize(self) -> None:
        ...
        self.repository.heartbeat(
            "B_COMPUTE", pid=os.getpid(), state="STARTING", detail="database-only EMS"
        )
    ...
    def run_cycle(self, now: Optional[float] = None) -> Optional[int]:
        ...
        if (
            ...
            self.repository.heartbeat(
                "B_COMPUTE", pid=os.getpid(), state="RUNNING",
                detail="waiting for dispatch period",
            )
            return None
        ...
        if state is None:
            ...
            self.repository.heartbeat(
                "B_COMPUTE", pid=os.getpid(), state="WAITING", detail="no A state"
            )
        ...
        try:
            ...
            # A stale or invalid state must never be converted into a fresh
            # executable command.  Keep the previous command and log the gap.
            self._report_problem("dispatch_input_rejected", str(error))
            self.repository.heartbeat(
                "B_COMPUTE", pid=os.getpid(), state="WAITING", detail=str(error)
            )
            self._last_decision_at = current
            return None

        outbox_id, created = self.repository.queue_decision(
            decision.state, decision.result, executable=closed_loop
        )
        if created:
            mode = "closed_loop" if closed_loop else "open_loop"
            self.repository.record_log(
                "INFO", "decision_created",
        ...
        self.repository.heartbeat(
            "B_COMPUTE", pid=os.getpid(), state="RUNNING",
            detail=f"last outbox={outbox_id}; executable={closed_loop}",
        )
    ...
    def run(self, *, max_cycles: Optional[int] = None, idle_sleep_s: float = 0.2) -> None:
        ...
        try:
            ...
            self.repository.heartbeat(
                "B_COMPUTE", pid=os.getpid(), state="STOPPED", detail="process stopped"
            )
