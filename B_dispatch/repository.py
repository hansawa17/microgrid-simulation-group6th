...
CREATE TABLE IF NOT EXISTS physical_parameters (
 ...
 wind_rated_kw REAL NOT NULL DEFAULT 100.0,
 wind_cut_in_mps REAL NOT NULL DEFAULT 3.0,
 wind_rated_speed_mps REAL NOT NULL DEFAULT 12.0,
 wind_cut_out_mps REAL NOT NULL DEFAULT 25.0,
 pitch_min_deg REAL NOT NULL DEFAULT 0.0,
 pitch_max_deg REAL NOT NULL DEFAULT 90.0,
 wind_ramp_up_kw_s REAL NOT NULL DEFAULT 40.0,
 wind_ramp_down_kw_s REAL NOT NULL DEFAULT 60.0,
 diesel_min_kw REAL NOT NULL DEFAULT 20.0,
 diesel_max_kw REAL NOT NULL DEFAULT 120.0,
 diesel_ramp_up_kw_s REAL NOT NULL DEFAULT 30.0,
 diesel_ramp_down_kw_s REAL NOT NULL DEFAULT 40.0,
...
CREATE TABLE IF NOT EXISTS current_state (
 ...
 pitch_actual_deg REAL NOT NULL CHECK(pitch_actual_deg>=0 AND pitch_actual_deg<=90), wind_running INTEGER NOT NULL CHECK(wind_running IN(0,1)),
 fault INTEGER NOT NULL CHECK(fault IN(0,1)), diesel_running INTEGER NOT NULL DEFAULT 0 CHECK(diesel_running IN(0,1)), power_imbalance_kw REAL NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS dispatch_evaluation (
 id INTEGER PRIMARY KEY AUTOINCREMENT, command_id INTEGER NOT NULL REFERENCES dispatch_commands(id), target_unserved_kw REAL NOT NULL CHECK(target_unserved_kw>=0),
 target_surplus_kw REAL NOT NULL CHECK(target_surplus_kw>=0), actual_unserved_kw REAL, actual_surplus_kw REAL, evaluated_at_utc TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS wind_execution_evaluation (
 id INTEGER PRIMARY KEY AUTOINCREMENT,
 session_id TEXT NOT NULL,
 dispatch_command_id INTEGER NOT NULL REFERENCES dispatch_commands(id),
 outbox_id INTEGER,
 c_wind_action_seq INTEGER,
 dispatch_step INTEGER NOT NULL CHECK(dispatch_step>=0), feedback_step INTEGER NOT NULL CHECK(feedback_step>dispatch_step),
 dispatch_created_at_utc TEXT NOT NULL, feedback_sampled_at_utc TEXT NOT NULL, wind_action_applied_at_utc TEXT,
 response_latency_s REAL CHECK(response_latency_s IS NULL OR response_latency_s>=0),
 b_wind_enable INTEGER NOT NULL CHECK(b_wind_enable IN(0,1)), b_wind_target_kw REAL NOT NULL CHECK(b_wind_target_kw>=0),
 c_controller_wind_enable INTEGER NOT NULL CHECK(c_controller_wind_enable IN(0,1)), c_pitch_target_deg REAL NOT NULL CHECK(c_pitch_target_deg>=0 AND c_pitch_target_deg<=90),
 c_wind_available_kw REAL NOT NULL CHECK(c_wind_available_kw>=0), c_wind_operating_limit_kw REAL NOT NULL CHECK(c_wind_operating_limit_kw>=0),
 a_wind_running INTEGER NOT NULL CHECK(a_wind_running IN(0,1)), a_wind_actual_kw REAL NOT NULL CHECK(a_wind_actual_kw>=0), a_pitch_actual_deg REAL NOT NULL CHECK(a_pitch_actual_deg>=0 AND a_pitch_actual_deg<=90), a_fault INTEGER NOT NULL CHECK(a_fault IN(0,1)),
 start_stop_score REAL CHECK(start_stop_score IS NULL OR(start_stop_score>=0 AND start_stop_score<=100)), power_tracking_score REAL CHECK(power_tracking_score IS NULL OR(power_tracking_score>=0 AND power_tracking_score<=100)),
 pitch_response_score REAL CHECK(pitch_response_score IS NULL OR(pitch_response_score>=0 AND pitch_response_score<=100)), capability_safety_score REAL CHECK(capability_safety_score IS NULL OR(capability_safety_score>=0 AND capability_safety_score<=100)),
 total_score REAL CHECK(total_score IS NULL OR(total_score>=0 AND total_score<=100)), verdict TEXT NOT NULL, reason TEXT NOT NULL, evaluated_at_utc TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS dispatch_outbox (
 ...
 protocol_seq INTEGER CHECK(protocol_seq IS NULL OR protocol_seq>=0), command_id INTEGER REFERENCES dispatch_commands(id), ack_accepted INTEGER CHECK(ack_accepted IN(0,1)), ack_reason TEXT, detail TEXT,
 created_at_utc TEXT NOT NULL, claimed_at_utc TEXT, completed_at_utc TEXT, UNIQUE(session_id,state_step,executable)
);
...
CREATE TABLE IF NOT EXISTS process_status (
 process_name TEXT PRIMARY KEY, pid INTEGER, state TEXT NOT NULL, detail TEXT NOT NULL, heartbeat_at_utc TEXT NOT NULL
);
CREATE TABLE IF NOT EXISTS communication_config (
 id INTEGER PRIMARY KEY CHECK(id=1), host TEXT NOT NULL, port INTEGER NOT NULL CHECK(port BETWEEN 1 AND 65535), enabled INTEGER NOT NULL DEFAULT 1 CHECK(enabled IN(0,1)), updated_at_utc TEXT NOT NULL
);
"""

UNIFIED_PHYSICAL = {
 "wind_rated_kw": 100.0, "wind_cut_in_mps": 3.0, "wind_rated_speed_mps": 12.0, "wind_cut_out_mps": 25.0,
 "pitch_min_deg": 0.0, "pitch_max_deg": 90.0, "wind_ramp_up_kw_s": 40.0, "wind_ramp_down_kw_s": 60.0,
 "diesel_min_kw": 20.0, "diesel_max_kw": 120.0, "diesel_ramp_up_kw_s": 30.0, "diesel_ramp_down_kw_s": 40.0,
}
...
class EMSRepository:
    """Every public operation uses a short, independent SQLite transaction."""
    ...
    def initialize(self) -> None:
        with self.connection() as conn:
            ...
            now = utc_now()
            cols = ",".join(UNIFIED_PHYSICAL)
            qs = ",".join("?" for _ in UNIFIED_PHYSICAL)
    ...
    def _migrate_legacy(self, conn: sqlite3.Connection) -> None:
        for table in ("current_state", "state_history"):
            self._add_column(conn, table, "wind_available_kw", "wind_available_kw REAL NOT NULL DEFAULT 0") if "wind_available_kw" not in self._columns(conn, table) else None
            self._add_column(conn, table, "wind_operating_limit_kw", "wind_operating_limit_kw REAL NOT NULL DEFAULT 0") if "wind_operating_limit_kw" not in self._columns(conn, table) else None
            self._add_column(conn, table, "diesel_target_kw", "diesel_target_kw REAL NOT NULL DEFAULT 0") if "diesel_target_kw" not in self._columns(conn, table) else None
            self._add_column(conn, table, "diesel_running", "diesel_running INTEGER NOT NULL DEFAULT 0") if "diesel_running" not in self._columns(conn, table) else None
            self._add_column(conn, table, "power_imbalance_kw", "power_imbalance_kw REAL NOT NULL DEFAULT 0") if "power_imbalance_kw" not in self._columns(conn, table) else None
            self._add_column(conn, table, "received_age_s", "received_age_s REAL NOT NULL DEFAULT 0") if "received_age_s" not in self._columns(conn, table) else None
            self._add_column(conn, table, "controller_wind_enable", "controller_wind_enable INTEGER")
            self._add_column(conn, table, "pitch_target_deg", "pitch_target_deg REAL")
            self._add_column(conn, table, "last_wind_action_seq", "last_wind_action_seq INTEGER")
            self._add_column(conn, table, "last_wind_action_step", "last_wind_action_step INTEGER")
            self._add_column(conn, table, "wind_action_applied_at_utc", "wind_action_applied_at_utc TEXT")
            self._add_column(conn, table, "extension_status", "extension_status TEXT NOT NULL DEFAULT 'legacy_or_incomplete'")
        cols = self._columns(conn, "dispatch_commands")
        for name, sql in (("ack_accepted","ack_accepted INTEGER"),("ack_reason","ack_reason TEXT"),("ack_received_at_utc","ack_received_at_utc TEXT")):
            self._add_column(conn, "dispatch_commands", name, sql)
        for table in ("current_state", "state_history"):
            conn.execute(f"UPDATE {table} SET extension_status=CASE WHEN controller_wind_enable IS NOT NULL AND pitch_target_deg IS NOT NULL AND last_wind_action_seq IS NOT NULL AND last_wind_action_step IS NOT NULL AND wind_action_applied_at_utc IS NOT NULL THEN 'complete' ELSE 'legacy_or_incomplete' END")
    ...
    def set_runtime_config(self, *, poll_period_s: float = 1.0, dispatch_period_s: float = 5.0, closed_loop: bool = True, command_timeout_s: Optional[float] = 3.0, max_state_age_s: float = 2.0) -> None:
        ...
        if not math.isfinite(poll) or poll <= 0 or not math.isfinite(dispatch) or dispatch <= 0 or not math.isfinite(age) or age <= 0: raise ValueError("runtime periods and max_state_age_s must be positive finite numbers")
    ...
    def save_state(self, state: Mapping[str, object]) -> None:
        ...
        ext_status = str(state.get("extension_status") or "legacy_or_incomplete")
        ext = (None if state.get("controller_wind_enable") is None else int(bool(state["controller_wind_enable"])), state.get("pitch_target_deg"), state.get("last_wind_action_seq"), state.get("last_wind_action_step"), state.get("wind_action_applied_at_utc"))
        params = (state["session_id"],int(state["step"]),float(state["sim_time_s"]),float(state["wind_speed_mps"]),float(state["wind_available_kw"]),float(state["wind_operating_limit_kw"]),float(state["load_power_kw"]),float(state["wind_actual_kw"]),float(state["diesel_actual_kw"]),float(state["wind_target_kw"]),float(state["diesel_target_kw"]),float(state["pitch_actual_deg"]),int(bool(state["wind_running"])),int(bool(state["fault"])),int(bool(state["diesel_running"])),float(state["power_imbalance_kw"]),state["sampled_at_utc"],state["received_at_utc"],float(state["received_age_s"]),*ext,ext_status)
        with self.connection() as conn:
            ...
            qs = ",".join("?" for _ in range(25))
            conn.execute(f"INSERT INTO current_state(id,{cols}) VALUES(1,{qs}) ON CONFLICT(id) DO UPDATE SET "+",".join(f"{c}=excluded.{c}" for c in cols.split(",")), params)
            ...
            if exists is None: conn.execute(f"INSERT INTO state_history({cols}) VALUES({','.join('?' for _ in range(25))})", params)
    ...
    def claim_next_outbox(self)->Optional[sqlite3.Row]:
        with self.connection() as conn:
            ...
            row=conn.execute("SELECT * FROM dispatch_outbox WHERE status='pending' ORDER BY id LIMIT 1").fetchone()
    ...
    def recover_abandoned_outbox(self)->int:
        with self.connection() as conn:
            cur=conn.execute("UPDATE dispatch_outbox SET status='delivery_unknown',detail='I/O process restarted while command was in sending state',completed_at_utc=? WHERE status='sending'",(utc_now(),)); return int(cur.rowcount)
    ...
    def record_evaluation(self,*,command_id:int,target_unserved_kw:float,target_surplus_kw:float,actual_unserved_kw:Optional[float]=None,actual_surplus_kw:Optional[float]=None)->int:
        with self.connection() as conn:
            cur=conn.execute("INSERT INTO dispatch_evaluation(command_id,target_unserved_kw,target_surplus_kw,actual_unserved_kw,actual_surplus_kw,evaluated_at_utc) VALUES(?,?,?,?,?,?)",(command_id,target_unserved_kw,target_surplus_kw,actual_unserved_kw,actual_surplus_kw,utc_now()));return int(cur.lastrowid)
    ...
    def record_wind_execution_evaluation(self, row: Mapping[str,object]) -> int:
        ...
        vals=tuple(row.get(k) for k in fields)
        placeholders=','.join('?' for _ in fields)
        with self.connection() as conn:
            cur=conn.execute(f"INSERT INTO wind_execution_evaluation({','.join(fields)}) VALUES({placeholders}) ON CONFLICT(dispatch_command_id) DO UPDATE SET "+','.join(f"{f}=excluded.{f}" for f in fields if f not in {"dispatch_command_id"}),vals)
            return int(cur.lastrowid or conn.execute("SELECT id FROM wind_execution_evaluation WHERE dispatch_command_id=?",(row["dispatch_command_id"],)).fetchone()[0])
