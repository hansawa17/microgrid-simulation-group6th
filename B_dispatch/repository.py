...
CREATE TABLE IF NOT EXISTS current_state (
    ...
    wind_actual_kw REAL NOT NULL CHECK (wind_actual_kw >= 0),
    diesel_actual_kw REAL NOT NULL CHECK (diesel_actual_kw >= 0),
    wind_target_kw REAL NOT NULL CHECK (wind_target_kw >= 0),
    pitch_actual_deg REAL,
    wind_running INTEGER NOT NULL CHECK (wind_running IN (0, 1)),
    fault INTEGER NOT NULL CHECK (fault IN (0, 1)),
    received_at_utc TEXT NOT NULL,
    received_age_s REAL NOT NULL CHECK (received_age_s >= 0)
);
...
class EMSRepository:
            ...
            conn.execute(
                """INSERT INTO dispatch_parameters
                   ...
                   ON CONFLICT(id) DO UPDATE SET
                     wind_min_kw=excluded.wind_min_kw,
                     wind_max_kw=excluded.wind_max_kw,
                     diesel_max_kw=excluded.diesel_max_kw,
                     reserve_kw=excluded.reserve_kw,
        ...
        with self.connection() as conn:
            conn.execute(
                """INSERT INTO ems_runtime_config
                   ...
                   VALUES (1, ?, ?, ?, ?, ?)
                   ON CONFLICT(id) DO UPDATE SET
                     poll_period_s=excluded.poll_period_s,
                     dispatch_period_s=excluded.dispatch_period_s,
                     closed_loop=excluded.closed_loop,
                     command_timeout_s=excluded.command_timeout_s,
                     updated_at=excluded.updated_at""",
    ...
    def save_state(self, state: Mapping[str, object]) -> None:
        required = (
            ...
            "wind_actual_kw", "diesel_actual_kw", "wind_target_kw", "wind_running", "fault",
        ...
        with self.connection() as conn:
            conn.execute(
                """INSERT INTO current_state
                   (id, session_id, step, sim_time_s, wind_speed_mps, load_power_kw,
                    wind_actual_kw, diesel_actual_kw, wind_target_kw, pitch_actual_deg,
                    wind_running, fault, received_at_utc, received_age_s)
                   VALUES (1, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                   ON CONFLICT(id) DO UPDATE SET
                     session_id=excluded.session_id, step=excluded.step,
                     sim_time_s=excluded.sim_time_s, wind_speed_mps=excluded.wind_speed_mps,
                     load_power_kw=excluded.load_power_kw, wind_actual_kw=excluded.wind_actual_kw,
                     diesel_actual_kw=excluded.diesel_actual_kw, wind_target_kw=excluded.wind_target_kw,
                     pitch_actual_deg=excluded.pitch_actual_deg, wind_running=excluded.wind_running,
                     fault=excluded.fault, received_at_utc=excluded.received_at_utc,
                     received_age_s=excluded.received_age_s""",
                values[:9] + (state.get("pitch_actual_deg"),) + values[9:],
            )
            conn.execute(
        ...
        with self.connection() as conn:
            cur = conn.execute(
                """INSERT INTO dispatch_commands
                   (session_id, step, sim_time_s, source, seq, wind_target_kw,
                    diesel_target_kw, wind_enable, diesel_enable, status, reason, created_at_utc)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    command["session_id"], command["step"], command["sim_time_s"],
                    command.get("source", "B"), command["seq"], command["wind_target_kw"],
                    command["diesel_target_kw"], int(command["wind_enable"]),
                    int(command["diesel_enable"]), command.get("status", "generated"),
                    command.get("reason", ""), command.get("created_at_utc", utc_now()),
                ),
            )
        ...
        with self.connection() as conn:
            conn.execute(
                """INSERT INTO event_log
                   (level, event_type, message, session_id, step, created_at_utc)
                   VALUES (?, ?, ?, ?, ?, ?)""",
