...
class MainWindow(_LegacyMainWindow):
    ...
    def __init__(self) -> None:
        ...
        self.host.setText(str(comm["host"]))
        self.port.setValue(int(comm["port"]))
        self.auto_dispatch = bool(self.params["closed_loop"])
        self.auto_box.blockSignals(True)
        self.auto_box.setChecked(self.auto_dispatch)
        self.auto_box.blockSignals(False)
        self.safe_label.setText(
            "正式模式：GUI 仅写入 ems.db；B_COMPUTE 生成调度，B_IO 独占 TCP、核对状态并等待 ACK。"
        )
        self.repo.heartbeat(
            "B_GUI", pid=os.getpid(), state="RUNNING", detail="database-only operator GUI"
        )
        self.poll_socket()
        self.log("GUI 启动：数据库模式；本窗口不直接打开 TCP，也不执行自动调度")

    def monitor_page(self) -> QtWidgets.QWidget:
        page = super().monitor_page()
        layout = page.layout()
        card = self.make_card()
        box = QtWidgets.QVBoxLayout(card)
        title = QtWidgets.QLabel("RTU / SCADA 四遥实时点表")
        title.setProperty("cardtitle", True)
        box.addWidget(title)
        self.scada_table = QtWidgets.QTableWidget(0, 6)
        self.scada_table.setHorizontalHeaderLabels(
            ["类别", "点名", "值", "单位", "数据时刻", "说明"]
        )
        self.scada_table.setAlternatingRowColors(True)
        self.scada_table.horizontalHeader().setStretchLastSection(True)
        self.scada_table.setMinimumHeight(230)
        box.addWidget(self.scada_table)
        layout.insertWidget(max(0, layout.count() - 1), card)
        return page

    def refresh_state_views(self) -> None:
        super().refresh_state_views()
        self._refresh_scada_table()

    def _refresh_scada_table(self) -> None:
        if self.state is None or not hasattr(self, "scada_table"):
            return
        state = self.state
        sampled = state.sampled_at_utc or "--"
        rows = [
        ...
        try:
            with self.repo.connection() as conn:
                command = conn.execute(
                    "SELECT wind_enable,diesel_enable,status,created_at_utc "
                    "FROM dispatch_outbox ORDER BY id DESC LIMIT 1"
                ).fetchone()
            if command is not None:
                command_time = str(command["created_at_utc"])
                status = str(command["status"])
                rows.extend([
                    ("YK 遥控", "wind_enable", "合" if command["wind_enable"] else "分", "bool", command_time, f"本地命令状态={status}"),
                    ("YK 遥控", "diesel_enable", "合" if command["diesel_enable"] else "分", "bool", command_time, f"本地命令状态={status}"),
                ])
            else:
                rows.extend([
                    ("YK 遥控", "wind_enable", "--", "bool", "--", "尚无本地命令"),
                    ("YK 遥控", "diesel_enable", "--", "bool", "--", "尚无本地命令"),
                ])
        except Exception:
            rows.extend([
                ("YK 遥控", "wind_enable", "--", "bool", "--", "命令队列暂不可读"),
                ("YK 遥控", "diesel_enable", "--", "bool", "--", "命令队列暂不可读"),
            ])
        self.scada_table.setRowCount(len(rows))
        for row_index, row in enumerate(rows):
            for column_index, value in enumerate(row):
                self.scada_table.setItem(
                    row_index, column_index, QtWidgets.QTableWidgetItem(str(value))
                )
    ...
    def history_page(self) -> QtWidgets.QWidget:
        ...
        bar=self.make_card(); g=QtWidgets.QGridLayout(bar)
        g.addWidget(QtWidgets.QLabel('Session'),0,0); self.h_session=QtWidgets.QLineEdit(); g.addWidget(self.h_session,0,1)
        ...
        g.addWidget(QtWidgets.QLabel('起始 UTC'),0,2); g.addWidget(self.h_start,0,3); g.addWidget(QtWidgets.QLabel('结束 UTC'),0,4); g.addWidget(self.h_end,0,5)
        g.addWidget(QtWidgets.QLabel('最多行数'),1,0); self.h_limit=QtWidgets.QSpinBox(); self.h_limit.setRange(10,5000); self.h_limit.setValue(200); g.addWidget(self.h_limit,1,1)
    ...
    def _history_filter(self, time_column: str) -> tuple[str,list[object]]:
        start=self.h_start.dateTime().toUTC().toString("yyyy-MM-dd'T'HH:mm:ss.zzz'Z'")
        end=self.h_end.dateTime().toUTC().toString("yyyy-MM-dd'T'HH:mm:ss.zzz'Z'")
        if end < start: raise ValueError('历史查询结束时间不能早于起始时间')
        clauses=[f'{time_column} BETWEEN ? AND ?']; args: list[object]=[start,end]
        session=self.h_session.text().strip()
        session_column=(time_column.split('.',1)[0]+'.session_id') if '.' in time_column else 'session_id'
        if session: clauses.append(f'{session_column}=?'); args.append(session)
        return ' WHERE '+' AND '.join(clauses),args

    def refresh_history(self) -> None:
        try:
            self._ensure_db(); limit=int(self.h_limit.value())
            state_where,state_args=self._history_filter('received_at_utc'); cmd_where,cmd_args=self._history_filter('c.created_at_utc'); log_where,log_args=self._history_filter('created_at_utc')
            with self.repo.connection() as conn:
    ...
    # These methods override the legacy direct-TCP runtime.  The formal GUI
    # has one auditable integration boundary: ems.db.
    def _process_snapshot(self) -> dict[str, object]:
        return {str(row["process_name"]): row for row in self.repo.get_process_status()}
    ...
    def _heartbeat_is_recent(row: object | None, *, max_age_s: float = 4.0) -> bool:
        ...
        try:
            stamp = datetime.fromisoformat(
                str(row["heartbeat_at_utc"]).replace("Z", "+00:00")
            )
    ...
    def set_connection_state(self, connected: bool) -> None:
        ...
        self.mode_status.setText(
            "数据库闭环" if self.params.get("closed_loop") else "数据库开环"
        )
        enabled = bool(getattr(self, "_communication_enabled", False))
        self.connect_btn.setText("停用 B_IO 通信" if enabled else "启用 B_IO 通信")
        self.connect_btn.setProperty("kind", "danger" if enabled else "success")
        self.connect_btn.style().unpolish(self.connect_btn)
        self.connect_btn.style().polish(self.connect_btn)
        self.request_btn.setEnabled(enabled)
    ...
    def toggle_connection(self) -> None:
        ...
        try:
            ...
            enabled = not self._communication_enabled
            self.repo.set_communication_config(host=host, port=port, enabled=enabled)
        except Exception as exc:
            self.log(f"通信配置无效：{exc}")
            QtWidgets.QMessageBox.warning(self, "通信配置错误", str(exc))
            return
        self.host.setText(host)
        self.port.setValue(port)
        self._communication_enabled = enabled
        self.set_connection_state(False)
        action = "启用" if enabled else "停用"
        self.log(f"已{action} B_IO 通信配置：{host}:{port}")
        self.statusBar().showMessage(f"通信配置已写入 ems.db；由 B_IO {action}连接")

    def request_state(self) -> None:
        """State polling belongs to B_IO; the GUI never emits protocol frames."""
        self.statusBar().showMessage("B_IO 会按采集周期自动请求状态；GUI 不直接发送 TCP 报文")
        self.log("状态请求由 B_IO 自动执行，GUI 未直接访问网络")
    ...
    def poll_socket(self) -> None:
        ...
        try:
            ...
            if state is not None:
                key = (state.session_id, state.step, state.sampled_at_utc)
                self.state = state
                if key != self._last_db_state_key:
                    self._last_db_state_key = key
                    self.refresh_state_views()
                    self.log(
                        f"从 ems.db 读取 state session={state.session_id} step={state.step}"
                    )
                elif hasattr(self, "cards"):
                    self.cards["age"].set_value(f"{state.received_age_s:.2f}")
            config = self.repo.get_communication_config()
            self._communication_enabled = bool(config["enabled"])
            io_row = self._process_snapshot().get("B_IO")
            online = bool(
                io_row is not None
                and str(io_row["state"]) == "ONLINE"
                and self._heartbeat_is_recent(io_row)
            )
            self.set_connection_state(online)
            self.repo.heartbeat(
                "B_GUI", pid=os.getpid(), state="RUNNING",
                detail="database-only operator GUI",
            )
        except Exception as exc:
            text = str(exc)
            if text != self._last_diag_text:
                self._last_diag_text = text
                self.log(f"读取 ems.db 失败：{text}")
    ...
    def send_current(self) -> None:
        ...
        try:
            ...
            if not bool(runtime["closed_loop"]):
                QtWidgets.QMessageBox.information(
                    self, "当前为开环", "开环模式仅计算和展示，不生成可下发控制命令。"
                )
                return
            state = self.repo.get_current_grid_state()
            if state is None:
                raise RuntimeError("ems.db 尚无 A 实时状态")
            if state.received_age_s > float(runtime["max_state_age_s"]):
                raise RuntimeError(
                    f"状态已过期：{state.received_age_s:.2f}s > "
                    f"{float(runtime['max_state_age_s']):.2f}s"
                )
            self.state = state
            self.calculate_current()
            if self.last_decision is None:
                return
            outbox_id, created = self.repo.queue_decision(
                state, self.last_decision.result, executable=True
            )
            message = (
                f"调度已进入数据库队列 outbox={outbox_id}，等待 B_IO 下发"
                if created else
                f"该状态的调度已存在 outbox={outbox_id}，未重复下发"
            )
            self.log(message)
            self.statusBar().showMessage(message)
        except Exception as exc:
            self.log(f"调度入队失败：{exc}")
            QtWidgets.QMessageBox.warning(self, "调度未入队", str(exc))

    def _try_send_queued_dispatch(self) -> None:
        # Compatibility hook used by inherited widgets. It never touches TCP.
    ...
    def set_auto(self, checked: bool) -> None:
        ...
        try:
            runtime = self.repo.get_runtime_config()
            self.repo.set_runtime_config(
                poll_period_s=float(runtime["poll_period_s"]),
                dispatch_period_s=float(runtime["dispatch_period_s"]),
                command_timeout_s=runtime["command_timeout_s"],
                closed_loop=bool(checked),
                max_state_age_s=float(runtime["max_state_age_s"]),
            )
            self.params["closed_loop"] = bool(checked)
            self.auto_dispatch = bool(checked)
            self.set_connection_state(False)
            self.log(f"运行模式已切换为{'闭环' if checked else '开环'}；由 B_COMPUTE 执行")
        except Exception as exc:
    ...
    def runtime_tick(self) -> None:
        ...
        try:
            with self.repo.connection() as conn:
                row = conn.execute(
                    "SELECT * FROM dispatch_outbox ORDER BY id DESC LIMIT 1"
                ).fetchone()
            if row is None:
                return
            ...
            self.d_w.setText(f"{result.wind_target_kw:.2f} kW")
            self.d_d.setText(f"{result.diesel_target_kw:.2f} kW")
            self.d_unserved.setText(f"{result.target_unserved_kw:.2f} kW")
            self.d_surplus.setText(f"{result.target_surplus_kw:.2f} kW")
            self.d_enable.setText(
                f"Wind={result.wind_enable} / Diesel={result.diesel_enable}"
            )
            self.d_reason.setText(result.reason)
            self.ack_label.setText(
                f"outbox={row['id']} / {row['status']} / {row['ack_reason'] or '--'}"
            )
        except Exception as exc:
            self.log(f"调度队列刷新失败：{exc}")

    def update_comm_diag(self) -> None:
        if not hasattr(self, "comm_state"):
            return
        try:
            statuses = self._process_snapshot()
            io_row = statuses.get("B_IO")
            compute_row = statuses.get("B_COMPUTE")
            io_state = "未启动" if io_row is None else str(io_row["state"])
            compute_state = "未启动" if compute_row is None else str(compute_row["state"])
            self.comm_state.setText(f"B_IO={io_state} / B_COMPUTE={compute_state}")
            with self.repo.connection() as conn:
                counts = {
                ...
                latest = conn.execute(
                    "SELECT id,status,protocol_seq,ack_accepted FROM dispatch_outbox "
                    "ORDER BY id DESC LIMIT 1"
                ).fetchone()
            self.comm_pending.setText(
                f"pending={counts.get('pending', 0)} / sending={counts.get('sending', 0)}"
            )
            self.comm_full.setText("SQLite outbox / GUI 无套接字")
            self.comm_seq.setText(
                "--" if latest is None else str(latest["protocol_seq"] or "--")
            )
            self.comm_ack.setText(
                "--" if latest is None else
                f"outbox={latest['id']} / {latest['status']} / {latest['ack_accepted']}"
            )
        except Exception as exc:
            self.comm_state.setText(f"数据库诊断失败：{exc}")

    def closeEvent(self, event) -> None:
        try:
            self.repo.heartbeat(
                "B_GUI", pid=os.getpid(), state="STOPPED", detail="window closed"
            )
        finally:
            event.accept()
