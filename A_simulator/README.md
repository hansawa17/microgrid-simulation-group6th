...
- 实时与历史曲线横轴使用 `sampled_at_utc`；场景预览用打开/初始化时的 UTC 起点加 `sim_time_s` 形成显示时间，但模型和协议仍保留可复现的相对仿真时间。
- 报警与通信页展示本机 IPv4、监听地址和端口；TCP 服务运行期间锁定监听配置，停止服务后才允许修改，修改仅作用于下一次启动的 TCP 子进程。
...
- 旧数据库损坏或版本不兼容时，GUI 进入明确的 `grid.db 异常`只读提示状态，不覆盖原文件。
...
一键脚本从自身位置定位仓库；缺少 `.venv` 时调用 `scripts/bootstrap.ps1`，缺少数据库时按指定配置和场景初始化一次，然后启动 GUI。**已有 `grid.db` 会被原样复用，不会被初始化或覆盖。**脚本不会创建或覆盖 `config.local.json`，本地配置与运行数据库仍由 `.gitignore` 排除。
...
`-BindAddress/-Port` 只有显式提供时才覆盖配置文件值，并作为 GUI 的初始监听设置；也可在 TCP 停止时从界面修改。`-Db/-Config/-Scenario` 可使用仓库相对路径或绝对路径。
...
- 顶部时钟显示操作系统提供的当前 UTC；A 不在 GUI 主线程直接轮询公网 NTP，系统时间服务负责授时。
- `sim_time_s` 仍是 CSV 场景内的相对时间，用于插值和模型计算；真实运行/历史图横轴使用每一步唯一的 `sampled_at_utc`。
...
- 顶部时钟与实时/历史横轴对齐 UTC 授时；保留 `sim_time_s` 作为模型内部相对场景时间，控制顺序保持 `session_id + step + seq`。
