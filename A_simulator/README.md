...
- “数据库分类”页将 YC 遥测、YX 遥信、YT 遥调、YK 遥控、设备参数、环境与场景、仿真历史和运行日志分成八个页签；`scada_points` 已包含四种类别，目标/动作与 actual 分开显示。
- 历史页可把当前筛选日志导出为 JSONL，每条包含发生时间和对象，并可在 Windows 文件资源管理器中定位导出文件。
...
- TCP 服务启动、停止或子进程异常退出时会将 B/C 连接状态复位；主界面每 5 s 显示 B/C 的应用层 `Ping(心跳龄)`，记录超过 10 s 即显示离线/超时，即使旧数据库位仍为 connected 也不会永久保留“正常”。服务端实际 socket 空闲清理由 30 s 阈值执行。
- 场景编辑保留独立的 CSV 保存/另存流程，并提供“应用曲线到仿真”按钮：在 ready/running/paused 状态下原子更新 `grid.db` 中的风速和负荷值，计算进程不中断，并从下一执行时刻生效。界面可独立配置开始时刻、结束时刻和计算步长并重采样曲线；此类时间轴变化须保存 CSV 并重新初始化 `grid.db`。
- 协议/数据库/进程异常会写日志并显示非阻塞错误弹窗，主界面轮询与绘制继续运行；`scripts/fault_injection/` 提供坏帧、断线/空闲和非法输入副本脚本。
...
CSV 表头固定为 `step,sim_time_s,wind_speed_mps,load_power_kw`。从 CSV 加载曲线后，在界面拖动曲线并松开鼠标会原子写回当前 CSV；也可点击“保存当前 CSV”或“另存 CSV”。这套 CSV 流程与运行期应用相互独立：点击“应用曲线到仿真”后，界面通过后台任务把风速/负荷值原子写入当前 `grid.db`，仿真无需暂停或重读文件，下一次计算事务自动读取完整的新曲线。数据库快照没有源 CSV，编辑后仍须另存才能获得 CSV 文件。

运行期应用只接受与当前会话完全相同的 `step/sim_time_s` 时间轴和点数；如修改仿真总时长、时间轴或点数，仍须重新初始化数据库。每次有效热更新写入 `scenario_runtime_updated` 日志，记录修改点数以及实际生效的下一 `step/sim_time_s`。
...
- “应用曲线到仿真”作为独立按钮与“保存当前 CSV”并列，应用数据库曲线时不隐式代替或取消原 CSV 写回流程。
- GUI 使用独立后台任务提交短 SQLite 事务；计算进程不停止，事务边界保证每一步只会看到完整旧曲线或完整新曲线。
- 新风速/负荷值从提交事务之后的下一执行时刻生效；当前状态和既有历史不回写，并记录 `scenario_runtime_updated` 追溯日志。
- 运行期仅允许修改纵轴值；时间轴、点数和总时长变化继续要求重新初始化数据库。
...
- “启动（含 TCP）”和“继续”会先按界面当前 bind/port 确保 TCP 子进程启动，再启动仿真计算；若数据库或监听配置无效，仿真不会伪装成已运行。
- B 仍以 1 s 状态请求作为主要应用层心跳；30 s 只用于清理真正无报文的连接，不是控制指令有效期。
- 真实跨电脑局域网、公网 raw TCP 隧道与 Windows 防火墙仍需在目标环境验证。

### 2026-09-10 · A 新版验收整改

- 数据库分类页明确分开 YC/YX/YT/YK、设备参数、环境/场景、仿真历史和运行日志，并补齐 A 的 YK 当前控制点。
- 历史页加入 UTC+8 起止时刻筛选，同一闭区间加载 SCADA 状态、运行日志和场景/控制/参数追溯；日志可导出并在资源管理器定位。
- 场景编辑加入仿真总时长修改，B/C 状态按 5 s 周期显示应用层心跳龄并覆盖陈旧的 connected 标志。
- `grid.db` 升至 v6，原子保存最近有效 C 风机动作的 seq、应用 step 和应用时间，A 的 state 已发送五个冻结扩展字段；`wind_action.payload` 仍保持原四字段。
- 增加故障注入脚本和非阻塞告警弹窗。当前验证仍为 PC 软件自动测试，不代表 STM32/Wi-Fi/UART 或跨电脑网络已实测。
