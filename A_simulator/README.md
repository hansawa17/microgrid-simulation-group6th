...
- 场景编辑保留独立的 CSV 保存/另存流程，并新增“应用曲线到仿真”按钮：在 ready/running/paused 状态下原子更新 `grid.db` 中的风速和负荷值，计算进程不中断，并从下一执行时刻生效。运行期不允许改变时间轴或点数；仿真总时长和时间轴修改仍须保存 CSV 并重新初始化 `grid.db`。
