...
| state | A → B/C | sampled_at_utc、wind_speed_mps、load_power_kw、wind_available_kw、wind_operating_limit_kw、wind_target_kw、wind_actual_kw、diesel_target_kw、diesel_actual_kw、pitch_actual_deg、wind_running、diesel_running、fault、power_imbalance_kw、next_command_seq；整改扩展见下文 |
...
超时或断线需标记离线、写日志并按确认的安全策略处理，不得静默把旧值标为新状态。A 当前软件服务的 TCP 空闲读超时为 30 s；服务启动、停止及 GUI 子进程异常退出都会复位 B/C 在线标志。该值是连接状态检测实现值，不替代仍待三方确认的控制命令有效期。
