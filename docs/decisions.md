...
- 参与：A 模块；不修改 A/B/C 公共协议、字段单位或计算所有权。
- CSV 保存与运行期应用保持为两个独立操作。“保存当前 CSV/另存 CSV”继续管理可复现场景文件；新增“应用曲线到仿真”只更新 A 本机 `grid.db` 的运行场景副本。
- 在 ready/running/paused 状态下，GUI 通过后台任务提交短 SQLite 事务，原子更新全部变化的风速/负荷点。计算进程不暂停；与单步计算竞争写锁时，任一计算步只看到完整旧曲线或完整新曲线。
- 生效边界定义为更新事务提交后的下一执行时刻。更新不改写当前状态和既有历史，日志记录 `scenario_runtime_updated`、修改点数及下一 `step/sim_time_s`，用于验收追溯。
- 为保持当前会话的时间语义，运行期只允许改变 `wind_speed_mps/load_power_kw`，必须保持点数及 `step/sim_time_s` 时间轴不变。总时长、点数或时间轴变化仍须保存并重新初始化数据库。
- 验证范围为 PC 软件自动测试：覆盖运行中更新、下一步生效、状态保持 running、时间轴变化拒绝、GUI 后台按钮路径与原 CSV 保存回归；未涉及 STM32/Wi-Fi/UART 或跨电脑硬件验证。
...
- 该决策当时仅冻结规范并生成 B/C 工作流提示词；A 的后续实现范围见下一节，B/C 数据库迁移及硬件验证仍不得据此宣称已完成。

## 2026-09-10：A 新版验收表整改实现

- A `grid.db` 升至 schema v6；`control_state` 增加最近有效 C `wind_action` 的 seq、应用 step 和应用 UTC 时间。合法动作在原事务中同时写入动作值与追踪元数据，重复报文不刷新元数据；新会话或 C 物理参数变化导致缓存动作失效时一并清空。
- A 的 `state.payload` 已实现冻结的五个扩展字段，但公共 `version` 仍为1，C→A `wind_action.payload` 仍严格保持原四字段。B/C 的兼容接收与数据库扩展仍按各自工作流完成，未宣称三方已联调。
- A `scada_points` 明确映射 YC=`telemetry`、YX=`signal`、YT=`setpoint`、YK=`control`，UI 分页展示四遥、设备参数、环境/场景、仿真历史和日志。B 目标、C 动作与 A actual 不混用。
- 场景编辑新增总时长调整，等比例重排当前 CSV 时间轴；修改只进入编辑场景，必须保存并重新初始化数据库，避免运行中静默改库。
- 历史页按北京时间输入起止时段，转换为 UTC 闭区间后由后台线程使用独立短连接查询状态、日志和场景/控制/参数追溯；曲线显示变量、单位与 UTC+8 时间信息，日志导出包含 occurrence time 和 object。
- B/C 连接健康每 5 s 刷新，界面显示的是应用层心跳年龄而非虚假的网络 RTT；超过 10 s 即覆盖陈旧 connected 记录显示离线/超时，服务端 socket 的 30 s 空闲清理不变。
- `scripts/fault_injection/` 增加协议坏帧、连接断开/空闲和非法输入副本；GUI 对新增 WARNING/ERROR 使用非阻塞弹窗，损坏数据库只提示一次且允许界面继续运行。
- 当前验证范围是 PC 端软件；跨电脑 TCP、FRP、STM32/Wi-Fi/UART 和真实设备动作仍须按验收记录模板实测。

## 2026-09-11：beta0.4 发布范围

- beta0.4 汇总 A 运行期场景曲线热更新、B GUI 直连非阻塞通信及参数刷新修复、C 开环不发送 `wind_action` 与运行期通信超时整改。
- B 接收 A 转发的参数快照后继续写入本机 `ems.db`；GUI 将仓储返回的 `sqlite3.Row` 显式转换为字典后刷新只读物理参数控件。该修复不改变公共 `parameter_update` 报文、字段所有权或控制权。
- C 开环仅消费 A 的 state 并继续轮询，控制计算结果仍经 `$WIND2` 上报串口；闭环才发送 `wind_action`。切换到开环后不重发尚未确认的闭环动作。
- 发布前在 Python 3.11.14 / PyQt6 6.11.0 环境运行完整仓库自动测试，151 项全部通过；新增真实 SQLite 行对象的 B 参数刷新回归测试。
- 本版未重新交叉编译或烧录 STM32 固件，也未完成真实三机局域网、ESP8266、UART 和异常断线验收；beta0.4 标签不代表这些硬件项目已经验证。

## 2026-09-11：v1.0.2 验收修复与网络自愈

- C 的网络自愈保持 `B -> A <- STM32` 拓扑，不让 C 上位机代替 STM32 直连 A。固件启动时与连续 3 次失败后用 `AT+RST` 软复位 ESP8266，然后重新设置单连接、加入 Wi-Fi、确认非 `0.0.0.0` STA IP 并重建 A TCP。未配置 ESP8266 硬件 RST 引脚，因此软复位无响应的电源/硬件故障仍需现场处理。
- USART1 错误回调清理 ORE/NE/FE/PE，丢弃当前不完整 AT/IPD 数据并重新挂载中断接收。C 上位机串口工作线程在拔插后不退出，按 1 s 间隔重开同一 COM 口；若 Windows 将设备重枚为新 COM 号，由操作员在界面重选。
- A 只有 WARNING 及以上日志会进入告警弹窗。最后一条同源 B/C 连接断开、空闲超时或 socket 错误现统一写 WARNING；同源还有其他有效连接时不会误报整个端离线。故障注入 abrupt 模式用 TCP RST 确保走 socket error 路径。
- A 场景编辑增加显式开始/结束/步长配置，新时间轴以线性插值重采样风速和负荷。时间轴修改需重新初始化；只改纵轴时仍可运行期原子热更新，下一执行时刻生效。
- B 远端提交 `2175ac5` 调整五项只读评价中的供需平衡阈值为 `<=3/<=10/<=50/>50 kW` 对应 `100/80/60/40`；评价仍不参与调度或改写数据。
- 本次不改变 TCP version 1、字段单位、参数所有权或 actual/target 计算边界。PC 测试不代表 STM32 固件已交叉编译或断线实测，后者使用 `docs/v1.0.2/06-onsite-demo-runbook.md` 现场记录。

## 2026-09-11：v1.0.2 后续短抖动容错修正

- 现场观察的 1–2 s 界面变化不能直接当作网络 RTT；A 仿真、B 轮询、C 控制均有 1 s 周期，端到端显示延迟会叠加。真实 RTT 需用带时间戳的 state request/response 或网络工具测量。
