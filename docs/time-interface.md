...
状态：三方基础开发接口基线。A 已完成状态、数据库和 TCP 输出适配；综合监控 UI 读取 UTC 时间戳并统一换算为 UTC+8 北京时间显示，B/C 按各自开发进度适配。本文补充 `common/protocol.md`，不改变 A/B/C 的控制权划分。
...
这里“实时对齐”是显示和历史关联规则，不是删除相对仿真时间：A 的实时/历史图横轴读取 `sampled_at_utc` 后换算为 UTC+8，模型仍使用 `sim_time_s` 读取确定性的 CSV 场景。两者必须同时保留。
...
A UI 顶部时钟显示 UTC+8 北京时间；实时曲线和历史曲线直接解析 `state_history.sampled_at_utc`，再以 `HH:mm:ss` 等 UTC+8 标签显示。UI 不得改写原始 UTC 值，也不得用刷新时刻、数据库查询时刻或本机收到 B/C 报文的时刻覆盖状态采样时间。
...
- A 初始化场景时校验全部字段并将曲线写入 `grid.db`；运行中不得临时随机生成另一条未落盘曲线。操作员可显式把编辑后的风速/负荷值原子应用到 `grid.db`，从事务提交后的下一执行时刻生效；当前状态和既有历史不回写，时间轴、点数及总时长变化仍须重新初始化。
...
A UI 在尚未生成真实状态的场景预览中，以本次打开或初始化场景时的 UTC 作为内部起点，再加每个点的 `sim_time_s` 并换算为 UTC+8 日期时间标签。该派生时间仅用于让预览与实时图的视觉形式一致，不写回 CSV，也不得冒充运行后的 `sampled_at_utc`。
...
- A：`grid.db`、状态模型和 TCP state 已包含两个能力字段；字段值由 C 计算，A 校验、存储并转发。授时、`sampled_at_utc` 和 CSV 相对时间曲线已完成；综合监控与历史图使用 UTC 数据并以 UTC+8 横轴显示，场景预览保留派生时间标识。
...
- 顶栏北京时间每秒刷新；明确标注 `UTC+8 北京时间`，数据库及协议字段仍明确为 UTC。
- 运行监控的风速和功率曲线使用同一批 `state_history` 断面和同一条 `sampled_at_utc` 横轴，不能因两个控件各自取时而错位。
- 历史表显示完整的 UTC+8 换算时间；图表可缩写标签，原始 RFC 3339 UTC 仍保存在数据库和协议状态中，可按 `session_id + step` 追溯。
