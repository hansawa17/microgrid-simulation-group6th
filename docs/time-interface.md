...
状态：三方基础开发接口基线，代码尚待各模块分别实现。本文补充 `common/protocol.md`，不改变 A/B/C 的控制权划分。
...
- A：扩展 `grid.db` 状态/历史结构和状态模型；统一读取一次 UTC 并写入 `state.payload.sampled_at_utc`；CSV 继续保存相对时间曲线。
