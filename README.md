...
### v1.1.0 beta1（2026-09-11）

- A：GUI、仿真计算和 TCP server 生命周期解耦。计算与通信分别运行在独立 Python 进程中，关闭 GUI 后继续仿真和通信，重开 GUI 后依据同一 `grid.db` 与后台心跳恢复监控。
- A：每个数据库的计算/TCP 服务分别使用系统文件锁保证单实例，增加 PID、端点、心跳、显式安全停止和 Windows 心跳文件竞争重试。
- B：修复 A 柴油机上下限虽能显示、调度却可能复用旧决策的问题；参数变化会立即重建约束、废弃缓存并在下一次 1 s 调度检查中按新上下限发送。
- 验证边界：CPython 3.11.14 / PyQt6 6.11.0 下 PC 自动化回归 **168 项全部通过**；新增后台计算持续推进、独立 TCP 响应及 GUI 关闭不暂停测试。STM32 实物与三机网络仍需现场验证。
...
- B：后台 I/O 路径补齐 `wind_execution_evaluation.evaluated_at_utc`，避免非空字段缺失导致评价写库失败。
- 测试：新增 GUI 调度→ACK→后续反馈评价链和后台评价时间戳回归；Python 3.11.14 环境下完整 PC 自动测试 **155 项全部通过**。
- 版本说明：已发布的 `v1.0.0` 标签保持不变，本修复按语义化版本发布为 `v1.0.1`；`docs/v1.0.0/` 的完整验收与部署文档继续适用。
...
- B：正式入口继续由 GUI 直接持有 TCP，收发与 ACK 处理保持非阻塞；修复 A 参数快照写入 `ems.db` 后，真实 `sqlite3.Row` 无法刷新参数控件的问题，并增加回归测试。
- C：开环模式只读取 A 的 `state` 并通过串口展示计算结果，不再发送可执行 `wind_action`；切换开环后丢弃未确认的挂起动作。state/ACK 超时使用运行期 `c_timeout_s`。
- 联调：A/B、A/C 软件 socket 路径、协议分包/粘包、命令 ACK、参数持久化与 GUI 离屏构造均纳入自动测试。
- 验证范围：Python 3.11.14 / PyQt6 6.11.0 环境下完整 PC 自动测试 **151 项全部通过**；C 固件未在本版重新交叉编译、烧录，真实三机局域网、STM32/Wi-Fi/UART 和异常断线仍需现场验收。
...
A 的计算循环与 TCP server 现在是两个独立后台 Python 进程。关闭 GUI 只关闭显示和控制窗口，不会暂停仿真或断开 B/C；再次启动 GUI 会自动读取原 `grid.db` 和后台心跳。现场需要显式停止时执行 `.\.venv\Scripts\python.exe -m A_simulator services stop --role all`，查看状态执行同命令并把 `stop` 改为 `status`。完整说明见 `A_simulator/README.md`。
...
- 场景编辑可修改仿真总时长；另提供与 CSV 保存平行的运行期曲线应用按钮，可在不中断仿真的情况下原子更新风速/负荷值并从下一执行时刻生效。B/C 连接每 5 s 显示应用层心跳龄并对陈旧记录强制显示超时，协议/数据库错误以非阻塞弹窗提示。
...
- 新增独立“应用曲线到仿真”按钮，不改变原有自动写回、保存和另存 CSV 行为。
...
修订版在 Python 3.11.14 / PyQt6 6.11.0 环境执行完整仓库测试，**155 项全部通过**。公网三机、STM32/Wi-Fi/UART 和真实设备动作没有在本次 PC 整改中重新验证。
