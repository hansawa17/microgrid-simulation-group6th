...
- **同一时刻只允许一个未完成的 `state_request`**：B 记录待处理请求序号，重复轮询不会连续发送新的状态请求。
- `poll_state()` 会持续消费 A 返回的 TCP 帧，直到获得新的 `state`；期间到达的 ACK 不会被静默丢弃。
- **dispatch 发送后立即等待并消费对应 ACK**，同一时刻只允许一个待确认 dispatch。
- ACK 在 B 客户端内存中保留，可供上层联调检查；`accepted=true` 只表示 A 接收/校验，不表示 actual 已达到目标。
- TCP 断线/超时仍向上层抛出，不把旧状态冒充新状态。
- `test_tcpB.py` 新增“未决 state request 不重复发送”和“dispatch 消费匹配 ACK”的回归测试。

**风机能力字段已进入协议草案。** `wind_available_kw` 表示风速功率曲线给出的资源能力；`wind_operating_limit_kw` 表示进一步考虑 C/STM32 启停许可、保护/故障、当前桨距和设备上限后的稳态运行上限，且不受 B 当前目标、B 当前启停或实际爬坡影响。B 后续使用 `wind_operating_limit_kw` 约束风机目标，不能再用 `wind_actual_kw` 替代。本次仅完成协议文档；A 状态/TCP、B 模型/调度/数据库和 STM32 解析仍需同步实现后才能宣称联调完成。

### 2026-09-07 · 环境与时间接口

- PC 端 Python 统一为 **3.11.x**，Qt 绑定统一为 **PyQt6 6.11.0**。
- 根目录使用 `.python-version` 和 `requirements.txt` 声明环境；每台电脑创建自己的 `.venv`。
- A 产生 `sampled_at_utc`，B/C 原样保存并另记接收时间。
...
B 已具备调度基础、闭环核心、本地数据库、周期 runtime、**B-owned TCPB** 和服务层桥接，并已针对 A 第一轮 TCP 对齐处理“请求/ACK 积压”风险。当前仅修改 B；尚未声称 A/B 实机 TCP 联调通过。
...
- `wind_available_kw` 是风资源能力，`wind_operating_limit_kw` 是运行约束后的稳态上限，`wind_target_kw` 是 B 目标，`wind_actual_kw` 是 A 实际输出，四者不得混用。
